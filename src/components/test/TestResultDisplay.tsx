...
function Section({
  ...
  return (
    <div className="rounded-2xl border border-white/8 bg-white/[0.025] overflow-hidden">
      {/* Section header */}
      <div className={`px-5 py-4 border-b border-white/8 flex items-center justify-between ${color}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs opacity-60 mt-0.5 font-normal">{directions}</p>
          </div>
        </div>
...
// -- Multiple Choice --
function MCQuestion({ q, n, showAnswer }: { q: MultipleChoiceQuestion; n: number; showAnswer: boolean }) {
  return (
              ...
              >
                <span
                  className={[
                    ...
                    isCorrect ? "bg-emerald-500/25 text-emerald-300" : "bg-white/8 text-slate-500",
                  ].join(" ")}
                >
                  {letter}
                </span>
...
// -- Fill in the Blanks --
function FIBQuestion({ q, n, showAnswer }: { q: FillInTheBlanksQuestion; n: number; showAnswer: boolean }) {
  return (
    <div className="flex gap-3">
      <QNum n={n} />
      <div className="flex-1">
        ...
        {showAnswer && (
          <p className="mt-1.5 text-xs text-brand-light font-medium">
            Answer: <span className="underline underline-offset-2">{q.answer}</span>
          </p>
        )}
      </div>
    </div>
...
// -- Enumeration --
function EnumQuestion({ q, n, showAnswer }: { q: EnumerationQuestion; n: number; showAnswer: boolean }) {
  return (
    <div className="flex gap-3">
      <QNum n={n} />
      <div className="flex-1">
...
// -- Essay --
function EssayQuestion({ q, n, showAnswer }: { q: EssayQuestion; n: number; showAnswer: boolean }) {
  return (
    <div className="flex gap-3">
      <QNum n={n} />
      <div className="flex-1">
        ...
        {q.hint && !showAnswer && (
          <p className="mt-2 text-xs text-slate-600 italic">(Guide: {q.hint})</p>
        )}
      </div>
    </div>
  );
...
// -- Main Component --
export default function TestResultDisplay({ test, fileName }: TestResultDisplayProps) {
  ...
  return (
    <section className="w-full animate-[fade-up_0.4s_ease-out] space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-brand-light text-xs font-medium">Test Generated</span>
            </div>
            <h2 className="text-white font-bold text-xl leading-tight">{test.title}</h2>
      ...
      {showAnswerKey && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-amber-500/20 bg-amber-500/8">
          ...
          <p className="text-amber-300 text-xs">
            <strong>Teacher mode active</strong> — answers are highlighted. Hide key before distributing to students.
          </p>
        </div>
      )}
      ...
      {test.multipleChoice.length > 0 && (
        ...
        >
          {test.multipleChoice.map((q, i) => (
            <MCQuestion key={i} q={q} n={mcStart + i} showAnswer={showAnswerKey} />
          ))}
        </Section>
      )}
      ...
      {test.fillInTheBlanks.length > 0 && (
        ...
        >
          {test.fillInTheBlanks.map((q, i) => (
            <FIBQuestion key={i} q={q} n={fibStart + i} showAnswer={showAnswerKey} />
          ))}
        </Section>
      )}
      ...
      {test.enumeration.length > 0 && (
        ...
        >
          {test.enumeration.map((q, i) => (
            <EnumQuestion key={i} q={q} n={enumStart + i} showAnswer={showAnswerKey} />
          ))}
        </Section>
      )}
      ...
      {test.essay.length > 0 && (
        ...
        >
          {test.essay.map((q, i) => (
            <EssayQuestion key={i} q={q} n={essayStart + i} showAnswer={showAnswerKey} />
          ))}
        </Section>
      )}
    </section>
