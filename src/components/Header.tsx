...
function TokenBadge({ tokens }: { tokens: number }) {
  const color =
    tokens >= 2 ? "text-emerald-400 border-emerald-500/30 bg-emerald-500/10"
    : tokens === 1 ? "text-amber-400 border-amber-500/30 bg-amber-500/10"
...
function UserMenu({ name, image, tokens }: { name: string; image?: string | null; tokens: number }) {
          ...
          <div className="absolute right-0 top-full mt-2 w-52 z-50 rounded-2xl border border-white/8 bg-[#091818] shadow-2xl shadow-black/60 overflow-hidden">
            <div className="px-4 py-3 border-b border-white/5">
              ...
              <p className="text-slate-500 text-xs mt-0.5">
                {tokens > 0 ? `${tokens} token${tokens !== 1 ? "s" : ""} remaining` : "No tokens remaining"}
              </p>
            </div>
            {tokens === 0 && (
              <div className="px-4 py-2.5 bg-amber-500/8 border-b border-amber-500/15">
                <p className="text-amber-400 text-xs font-medium">Tokens exhausted</p>
                <p className="text-slate-400 text-xs mt-0.5">Upgrade to keep going</p>
              </div>
            )}
            <button
    ...
    <header className="fixed top-0 inset-x-0 z-50 border-b border-white/5 bg-[#040e0e]/80 backdrop-blur-md">
      <div className="mx-auto max-w-6xl px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-slate-400">
          <a href="/#features" className="hover:text-white transition-colors" suppressHydrationWarning>Features</a>
          <a href="/#how-it-works" className="hover:text-white transition-colors" suppressHydrationWarning>How it works</a>
        ...
        <div className="md:hidden flex items-center gap-3">
          {status === "authenticated" && session?.user && (
            <TokenBadge tokens={session.user.tokens ?? 0} />
          )}
          {status === "unauthenticated" && (
            <Link href="/analyze" className="px-3 py-1.5 rounded-lg bg-brand-dark text-white text-sm font-medium" suppressHydrationWarning>
              Get started
            </Link>
          )}
        </div>
